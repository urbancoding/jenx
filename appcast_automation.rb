...
class AppCast
  ...
  YAML_FOLDER_PATH = File.expand_path('.')
  ...
  def instantiate_appcast_variables
    ...
    @appcast_xml_path       = "#{YAML_FOLDER_PATH}/#{@appcast_proj_folder}/#{@appcast_xml_name}"
  ...
  def copy_archive_to_appcast_path
    begin
      FileUtils.cp(@archive_path, "#{YAML_FOLDER_PATH}/#{@appcast_proj_folder}")
    rescue
...
<rss version=\"2.0\" xmlns:sparkle=\"http://www.andymatuschak.org/xml-namespaces/sparkle\"  xmlns:dc=\"http://purl.org/dc/elements/1.1/\">
  <channel>
    <title>#{@proj_name.capitalize} ver. #{@version.chomp}</title>
